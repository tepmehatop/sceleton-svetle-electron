<script lang="ts">
	...
	let toastArr = [
		'',
		'Wow! полегче!',
		'Так и затыкать можно!',
		'Ты чего? в глаз попадешь!',
		'Может лучше регресс?',
		'Не тыкай много, мышка сломается',
		'M-M-M-M-MONSTER KILL!',
		'10 очков Слизерину',
		'Протестировано!',
		'Тут нет багов!',
		'До тебя тоже кто то тыкал и затыкал до смерти',
		'Ларри, мы тут не за этим!',
		'Я гусь, можно я до вас...',
		'Го в Леруа!',
		'Верните ДОРы!!!',
		'Кластер ломастер!',
		'Мокрые шишки',
		'Ловите яичного браканьера!',
		'Выкуплю Macbook не дорого 8-91......',
		'Видишь регресс? а он есть',
	...
	function toastBasic(): void {
		const t: ToastSettings = {
			message: toastArr[randomIntFromInterval(1, toastArr.length)],
			hideDismiss: true,
			timeout: 3000
		};
...
<AppShell>
	...
	<svelte:fragment slot="header">
		<div class="navbar bg-base-100 h-4">
			<button on:click={toastBasic}>
				<figure>
					<section class="img-bg" />
					<svg
						class="fill-token -scale-x-[50%] -scale-y-[-50%]"
			...
			<a class="btn btn-ghost normal-case text-lg" on:click={calcCount} href="/oldStubs">Старая заглушка {count}</a>
			<a class="btn btn-ghost normal-case text-lg" on:click={calcCount} href="/">Генератор потребностей {count}</a>
			<a class="btn btn-ghost normal-case text-lg" on:click={calcCount} href="/mockPage">Управление моком {count}</a>
...
<style lang="postcss">
	...
	figure svg,
	.img-bg {
		@apply w-64 h-64 md:w-20 md:h-20;
	}
	.img-bg {
		@apply absolute z-[-1] rounded-full blur-[180px] transition-all;
		animation: pulse 5s cubic-bezier(0, 0, 0, 1.5) infinite, glow 5s linear infinite;
	}
	@keyframes glow {
		4% {
